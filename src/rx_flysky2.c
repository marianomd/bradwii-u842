...
void initrx(void) {
    ...
    sethopping(usersettings.flysky_id);
    chancol = 0;
    nextchannel();
}
...
void bind() {
    ...
    while (1) {
        ...
        if (packet[0] == 170) // 170
        {
            ...
            if (i == 21) {
                //set found tx		
                usersettings.flysky_id = ((packet[1] << 0 | packet[2] << 8 | packet[3] << 16 | packet[4] << 24));
                break;
            }
        }
        A7105_Strobe(A7105_RX);
...
void readrx(void) {
    char mode = A7105_ReadRegister(A7105_00_MODE);
    if (mode & A7105_MODE_TRER_MASK) {// nothing received
        if (lib_timers_gettimermicroseconds(timeout_timer) > 28000) {// change channel in case there is no reception in it
#ifdef ANY_TX
            if (id == 0) { // we have no tx id
    ...
    if (mode & (1 << 6) || (mode & (1 << 5))) {// fec and crc check
...
#ifdef ANY_TX		
    if (id == 0) {// if we have no id save the found id here
        id = idreceived;
        sethopping(id);
    }
#endif
    if (usersettings.flysky_id != idreceived) {// different tx
        A7105_Strobe(A7105_RX);
        return;
    }
    decodepacket();
