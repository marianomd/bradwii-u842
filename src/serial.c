...
void evaluatecommand(char portnumber, unsigned char *data) {
    ...
    if (command == MSP_IDENT) { // send rx data
        ...
        sendandchecksumcharacter(portnumber, lib_fp_multiply(global.batteryvoltage,10<<FIXEDPOINTSHIFT)>>FIXEDPOINTSHIFT);
...
void serialcheckforaction(void) { // to be called by the main program every cycle so that we can check to see if we need to respond to incoming characters
#if (MULTIWII_CONFIG_SERIAL_PORTS & SERIALPORT0)
#ifdef SERIALTEXTDEBUG
    serialcheckportforactiontest(0);
#else
